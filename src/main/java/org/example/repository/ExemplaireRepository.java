...
public interface ExemplaireRepository extends JpaRepository<Exemplaire, Integer> {
    ...
    // Disponible par rapport au pret
    @Query("""
                SELECT e FROM Exemplaire e
                WHERE e.livre.id = :idLivre
                AND NOT EXISTS (
                    ...
                    AND p.rendu = false
                    AND (
                        :dateDebut BETWEEN p.dateDebut AND p.dateRetour
                    )
                )
            """)
    List<Exemplaire> findDisponiblesSelonPret(
            @Param("idLivre") int idLivre,
            @Param("dateDebut") LocalDate dateDebut);

    // Disponible par rapport aux reservations
    @Query("""
                SELECT e FROM Exemplaire e
                WHERE e.livre.id = :idLivre
                AND NOT EXISTS (
                    ...
                    AND r.livre = e.livre
                    AND (
                        :dateDebut BETWEEN r.dateReservation AND COALESCE(r.dateFinReservation, r.dateReservation)
                    )
                )
            """)
    List<Exemplaire> findDisponiblesSelonReservation(
            @Param("idLivre") int idLivre,
            @Param("dateDebut") LocalDate dateDebut);
